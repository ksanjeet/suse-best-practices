...
// Fabian Herschel, Lars Pinne, Sanjeet Kumar Jha
// 2022-06-10
...
for {saphana} scale-out system replication in a multi-target architecture where the automation of system replication between {saphana} first site to {saphana} second site  is managed {sles4sapReg} while there is an additional system replication to a third site and this system replication is outside the scope of automation by {sles4sapReg}.

High availability is an important aspect of running your mission-critical
{saphana} servers.

The {saphana} scale-out multi-target system replication is a replication of all data in
{saphana} to a second {saphana} system usually in a SYNC replication and to a third  {saphana} system usually in an ASYNC replication. The {saphana} itself replicates all of its data to a secondary {saphana} instance. It is an out-of-the-box, standard feature.
...
decision based on coded algorithms. The replication to the third hana system is a asynchronous replication and is outside the control of cluster.


=== Abstract

This guide describes planning, setup, and basic testing of {sles4sap} {prodNr} for "SAP HANA Scale-Out Multi-Target System Replication" scenario.
...
- 3-site cluster with disk-based SBD fencing and a 4th non-cluster site
...
This document describes how to set up {saphana} scale-out multi-target system replication with a cluster installed on two sites(and a third site which acts as majority maker) based on {sles4sap} {pn15} {psp15} and how to add an additional non cluster site for a multi target architecture.
...
We will first setup a {sles4sap} cluster controlling
two 'pairs' of {saphana} scale-out in a system replication configuration. We will next setup a third site which is outside SUSE HA cluster but acts as additional {saphana} target forming a multi target architecture.
...
In this guide we will cover scale-out scenario without standby nodes which means we will not cover the scenario where {saphana} is configured for host auto-failover.
...
NOTE: Although, it is possible to have standby nodes in scale-out multi-target setup, however this guide is created with a focus of ERP style setup and involves no standby nodes. All 3 sites have 2 nodes each.
...
The multi target architecture with a cluster organizing two {saphana} 'pairs' and a third site is quite complex. The installation
...
Site 3 is connected as an additional system replication target to the {SAPHANA} node running master nameserver. In case of a takeover, HANA can be configured in such a way that site 3 will automatically reregister itself to the correct site for system replication.
...
Site 3 can be configured to follow the primary {SAPHANA} after takeover. For this a configuration 'register_secondaries_on_takeover=true' needs to be added in the system_replication block of the global.ini file.
...
This step must be done on the two sites that will be part of the cluster. {SAPHANA} must be stopped to change the
...
It is again reminded that the new srHook script "SAPHanaSrMultiTarget.py" necessary for multi-target setup is not available in the installation ISO media and only available in update channels of SLES for SAP 15 SP3 or earlier. So, for a correctly working multi-target setup a full system patching is mandatory after registering the system to SCC, RMT or SUSE Manager. From SLES for SAP 15 SP4 onwards the "SAPHanaSrMultiTarget.py" will be included in the the ISO.
...
For a multi-target setup Site 3 can be configured to follow the primary {SAPHANA} after takeover. For this a configuration 'register_secondaries_on_takeover=true' needs to be added in the system_replication block of the global.ini file. This configuration needs to be added on the two {SAPHANA} sites which are in cluster.
...
----
# SAPHanaSR-ScaleOut needs for srHook
tstadm ALL=(ALL) NOPASSWD: /usr/sbin/crm_attribute -n hana_tst_site_srHook_*
----
===================================
...
[subs="specialchars,attributes"]
----
{mySite1FirstNode}:tstadm> cdtrace
{mySite1FirstNode}:tstadm> awk '/ha_dr_SAPHanaS.*crm_attribute/ \
     { printf "%s %s %s %s\n",$2,$3,$5,$16 }' nameserver_{mySite1FirstNode}.*
2022-07-26 12:34:04.476445 ha_dr_SAPHanaS...SFAIL
2022-07-26 12:53:06.316973 ha_dr_SAPHanaS...SOK
----
...
[[MultiTarget]] This chapter is divided into two sections. The first section describes the procedure to upgrade an existing scale-out performance optimized setup into a multi-target ready setup. The second section is about configuring the 3rd site in a multi-target setup after freshly installing the whole landscape using the procedures mentioned in the document so far.  
...
Register the master node on the third site with the source of the {saphana} system replication.
...
==== Adapt configurations on clustered HANA nodes.
We need to make 2 changes in the HANA configurations of the clustered HANA nodes. The first change is done in the 'nameserver.ini'. This change ensures that there is no second master candidate as this is an ERP style setup. By default during the HANA installation the second node in each site will be configured as the master candidate. We need to remove the second node from the line starting with 'master ='. 
...
The second change(if not already done in previous steps) needs to be done in the section system_replication of global.ini of the first two sites add the configuration:
...
Above configuration will ensure that whenever there is a takeover between site A and site B, site C will re-register and always sync with the correct source.
...
hanaso1 DEMOTED     2.0 2.2 online     slave:slave:worker:slave   140   WDF1 
hanaso2 DEMOTED     2.0 2.2 online     master1:master:worker:master 100   ROT1 
hanaso3 DEMOTED     2.0 2.2 online     slave:slave:worker:slave   80    ROT1 
...
Expected Observation: The master candidate node the secondary site is promoted as the new master. System Replication gets broken between the primary and secondary site. The source of system replication to the DR site changes to the new master.
 
* Power-off the worker node but not the master name server of the primary. 

Expected Observation: Cluster notices that the worker node at primary site is down. No impact on the system replication.
...
Expected Observation: The master candidate on the secondary site is promoted as new master. System replication gets broken between pimary and secondary site. The source of system replication to the DR site changes to the new master.
...
Expected Observation: Cluster notices the power down of the master candidate on secondary. System replication gets broken between primary and secondary.

* Power-off the worker node but not the master name server of the secondary. 

Expected Observation: Cluster notices the power down of the worker node on secondary site. No impact on the system replication.

* Re-join of a previously power-off cluster node.

Expected Observation: Cluster notices re-join of previously power-off node. If it was a worker node then there is no impact on the system replication. In case it was previously a master nameserver node then depending on the configuration it may automatically join as secondary or wait for a manual registration by admin.
...
Expected Observation: Cluster notices the nodes that are down. System replication gets broken.
...
The majority maker _hanamm_ does not run an {saphana} instance and therefore
