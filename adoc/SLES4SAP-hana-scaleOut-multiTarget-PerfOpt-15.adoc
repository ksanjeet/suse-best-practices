...
The {saphana} needs to be stopped at both sites  that will be part of the 
Linux cluster. At each site do the following:
[subs="specialchars,attributes"]
----
# su - {refsidadm}
~> sapcontrol -nr {refInst} -function StopSystem
...
NOTE: All nodes should be started in parallel. Otherwise unseen nodes might get
fenced.
