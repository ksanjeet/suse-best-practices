...
FuseML can run locally, on its own Kubernetes cluster, or alongside an AI/ML workload on a shared Kubernetes cluster. In this design we will use SUSE Rancher to deploy FuseML in SUSE RKE2. RKE2 uses a cutting edge architecture which adapts the prinicpals of containerization for Kuberetes cluster itself. RKE2 deploys as a single, static binary with all dependencies handled inside. Not only does this create fantastic portability, it also significantly enhances security. This design is part of what allows RKE2 to be FIPS compliant with no modifications.
