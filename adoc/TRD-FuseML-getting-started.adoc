...
// = {title}
= {centralProduct} with RKE2 - Getting Started

// SUSE RKE2 - FuseML
// :author: Alex Arnoldy
:revnumber: 0.0.1
:toc2:
:toc-title: FuseML with RKE2 - Getting Started
...
FuseML can run locally, on its own Kubernetes cluster, or alongside an AI/ML workload on a shared Kubernetes cluster. In this design we will use {rancher} to deploy FuseML in SUSE RKE2. RKE2 uses a cutting edge architecture which adapts the prinicpals of containerization for Kuberetes cluster itself. RKE2 deploys as a single, static binary with all dependencies handled inside. Not only does this create fantastic portability, it also significantly enhances security. This design is part of what allows RKE2 to be FIPS compliant with no modifications.
...
* name "fuseml-rke2-VM":
** 2 vCPUs, 8 GB RAM, 20GB virtual disk
** Virtual bridge/NAT network with access to the Internet
** {operatingsystem}
** Latest release of RKE2
...
{operatingsystem} is a minimalized, virtualization specific edition of {sles} that runs as a pre-installed operating system. Pre-configured images are available for KVM, Xen, VMware, OpenStack, and Hyper-V. The OpenStack image can be run on KVM or Xen but allows the use of cloud-init for customization during the initial boot sequence.
...
* After the container is running, execute the following command to get the bootstrap password:
----
...
NOTE: IF the virtual machines are behind a NAT router it may be necessary to configure port forwarding for ports 80 and 443 to the first VM. Additionally, an SSH tunnel can be created to access the virtual machines. In our case we used an SSH tunnel through the KVM host to access ports 80 and 443 on the first VM with the command `ssh -L localhost:8080:10.236.1.11:80 -L localhost:4443:10.236.1.11:443 -N sles@kvm-host1.sandbox.local`. This allowed us to browse the {rancher} server at https://localhost:4443.
