...
docker run \
	...
	--privileged \
  --name rancher \
  rancher/rancher -- -e CATTLE_BOOTSTRAP_PASSWORD=FuseML4Me!
----
...
NOTE: Newer versions of containerd require AppArmor installed for certain operating systems. Here were are using SLES JeOS, which has AppArmor available but not fully installed.
...
** The {k8s} deployment is complete when elements of all the deployments (coredns, local-path-provisioner, metrics-server, and traefik) show at least "1" as "AVAILABLE"
